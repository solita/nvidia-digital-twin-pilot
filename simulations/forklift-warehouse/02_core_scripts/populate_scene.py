...
if stage is None:
    print("[populate_scene] ERROR: No stage loaded — open scene_assembly.usd first.")
else:
    root_layer = stage.GetRootLayer()
    src_path = Sdf.Path(TEMPLATE_PRIM)

    if not stage.GetPrimAtPath(TEMPLATE_PRIM).IsValid():
        print(f"[populate_scene] ERROR: Template prim not found: {TEMPLATE_PRIM}")
    else:
        # ── Clone forklifts ───────────────────────────────────────────────────
        for fl in FORKLIFTS:
            ...
            if stage.GetPrimAtPath(str(dst_path)).IsValid():
                log(f"Prim already exists: {dst_path} — skipping clone")
            else:
